...
export default function ActivityChart({ data, days: propDays }: ActivityChartProps) {
  ...
  const fetchActivityData = async (timeRange: TimeRange = selectedTimeRange) => {
    try {
      setLoading(true);
      setError(null);
      ...
      if (result.success) {
        ...
        setChartDays(formatDays(activityData.days, timeRange));
      } else {
        setError('获取数据失败');
  ...
  const formatDays = (days: string[], timeRange: TimeRange): string[] => {
  ...
  return (
    <div className="relative">
      <div className="mb-4 flex items-center justify-between">
        <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300">活动趋势</h3>
        <div className="flex items-center gap-2">
          {Object.entries(timeRangeLabels).map(([value, label]) => (
            <button
              key={value}
              onClick={() => handleTimeRangeChange(value as TimeRange)}
              className={`px-3 py-1 text-xs rounded-md transition-all ${selectedTimeRange === value ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200 dark:bg-gray-700 dark:text-gray-300 dark:hover:bg-gray-600'}`}
            >
              {label}
            </button>
          ))}
