...
interface AccountMappingReferenceListProps {
  show: boolean;
  onClose: () => void;
  corpId: string;
  accountName?: string;
...
export default function AccountMappingReferenceList({
  show,
  onClose,
  corpId,
  accountName
}: AccountMappingReferenceListProps) {
    ...
    try {
      ...
      if (data.success) {
        const filteredMappings = data.data.filter(
          (mapping: MappingConfig) => mapping.corpId === corpId
        );
  ...
  return (
    <GenericReferenceDialog<MappingConfig>
      ...
      onClose={onClose}
      title="数据映射引用"
      subtitle={`${accountName || '企业微信账号'} · ${mappings.length} 个映射`}
      ...
      emptyState={{
        ...
        title: '暂无数据映射',
        description: '该企业微信账号尚未被任何数据映射引用'
      }}
      maxHeight="500px"
      maxWidth="2xl"
