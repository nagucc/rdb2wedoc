...
import { ConflictStrategy, SyncMode, IncrementalType, FieldConflictStrategy, MappingConfig, SyncJob } from '@/types';
...
export default function EditSyncJobPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (response.ok && result.success) {
        setSuccessMessage('同步作业更新成功！正在跳转...');
        
        setTimeout(() => {
          router.push('/sync-jobs');
        }, 1500);
      } else {
        const errorMessage = result.error || '更新失败，请重试';
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 dark:text-gray-400">加载同步作业配置中...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800">
      <Header 
        ...
        breadcrumbItems={[
          ...
          { title: '同步作业管理', href: '/sync-jobs' },
          { title: '编辑同步作业', href: `/sync-jobs/edit/${jobId}`, isActive: true }
        ]} 
      />
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="mt-1 text-sm text-blue-700 dark:text-blue-300">
                  修改同步作业的配置信息。更改将立即生效，但正在运行的作业将在下次执行时使用新配置。
                </p>
              </div>
            </div>
          </div>
