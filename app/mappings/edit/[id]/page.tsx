...
export default function EditMappingPage() {
  ...
  const handleAIMatch = async () => {
    ...
    // 如果正在加载文档字段，等待加载完成
    if (loadingDocumentFieldsRef.current) {
      try {
    ...
    // 如果文档字段为空，尝试先加载
    if (documentFields.length === 0 && !loadingDocumentFields) {
      try {
        ...
        setError('加载文档字段失败，无法进行AI匹配');
        return;
      }
    }

    if (documentFields.length === 0) {
      setError('目标文档字段为空，无法进行AI匹配');
      return;
    }

    const validDocumentFields = documentFields.filter(f => f.name && f.name.trim() !== '');
    if (validDocumentFields.length === 0) {
      setError('目标文档字段名称为空，无法进行AI匹配');
      return;
    }
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800">
      <Header 
        ...
        breadcrumbItems={[
          ...
          { title: '编辑数据映射', href: `/mappings/edit/${mappingId}`, isActive: true }
        ]} 
      />
                ...
                {fieldMappings.length === 0 ? (
                  ...
                  <div className="space-y-4">
                    {fieldMappings.map((mapping, index) => (
                      <div key={`${mapping.id}-${index}`} className="bg-gray-50 rounded-lg p-4 border border-gray-200 dark:bg-gray-700 dark:border-gray-600 transition-all duration-300 hover:shadow-md hover:border-indigo-200 dark:hover:border-indigo-700">
